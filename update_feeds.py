...
class MwmListParser(HTMLParser):
    """Extract article detail links from a 管理世界 issue-list page."""
...
class MwmDetailParser(HTMLParser):
    """Extract complete metadata from a 管理世界 article detail page."""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "p" and "titletext" in classes and not self.title_depth:
            self.title_depth = 1
        elif tag == "span" and "date" in classes and "mrl20" in classes and not self.author_depth:
            self.author_depth = 1
        elif tag == "span" and "date" in classes and not self.date_depth:
            self.date_depth = 1
        elif tag == "div" and "mrt20" in classes and not self.content_depth:
            self.content_depth = 1
    ...
    def metadata(self) -> dict[str, str]:
        ...
        author_text = clean_html_text(self.author_parts)
        content = clean_html_text(self.content_parts)
        author = re.sub(r"^作者\s*[：:]\s*", "", author_text).strip()
        citation_authors_match = re.search(
            r"文章刊发\s*[：:]\s*(.+?)：《",
            content,
        )
        if citation_authors_match:
            author = citation_authors_match.group(1).strip()
        ...
        issue_match = re.search(r"(\d{4}年第\d+期)", content)
...
def build_mwm_feed(
    ...
    """Build a feed from 管理世界's paginated current-issue pages."""
    ...
    for detail_link in detail_links:
        ...
        if not all(
            article.get(field)
            for field in ("title", "author", "published", "abstract")
        ):
