...
- 自动更新分为两条相互衔接的流程。《数量经济技术经济研究》和《管理世界》由 Gitee 国内节点在北京时间每月 1、4、7……日 05:30 左右采集，成功后立即把压缩并签名的内存数据发送给 GitHub；GitHub 收到事件后随即校验、生成 RSS 并发布，不需要等下一次定时任务，也不进行轮询。Gitee 不保存题录文件、不上传构建产物，日志也不输出标题、作者或摘要。
...
- GitHub 常规定时检查会刷新其负责订阅的 `lastBuildDate`，所以仓库出现新的自动提交不一定表示期刊新增了文章。Gitee 传入的两刊会比较条目的实际内容；完全相同的批次不会产生新提交。
...
| 数量经济技术经济研究 | [期刊官网 RSS](https://www.jqte.net/download_upload_file.aspx?file_name=/rss/sljjjsjjyj/cn/current.xml) | Gitee 国内节点直接读取官网当前期题录，在内存中拆分并去重作者、过滤非文献条目，再把链接精确转换到 `www.jqte.net` 官网文章页。数据经摘要校验和 HMAC 签名后发送到 GitHub；GitHub 复核期次、字段与链接并生成 RSS，既有 GUID 保持不变。 |
...
| 管理世界 | [期刊官网当期目录](http://www.mwm.net.cn/web/bqym)及逐篇详情页 | Gitee 国内节点遍历官网当期分页和文章详情；完整作者、期次与页码优先从官网标准引文提取，避免顶部“某某 等”造成作者缺失。过滤非文献后，每条均保留官网文章深链。数据经摘要校验和 HMAC 签名发送，GitHub 再复核期次、字段、链接和条目数量后生成 RSS。 |
...
目前两刊已经恢复为官网数据：Gitee 只承担国内网络采集和即时发送，GitHub 负责不可信输入校验、RSS 生成、版本保存与 Pages 发布。传输采用 SHA-256 摘要和独立 HMAC 密钥签名；GitHub 只接受指定账户发来的这两个期刊，拒绝过期、篡改、来源不明、字段异常或期次倒退的数据。Gitee 当前分支不再生成或提交期刊 XML；旧提交历史中曾经保存的文件不会因本次改造自动消失。
...
| 数量经济技术经济研究 | 2026 年第 7 期；2026-07-28 | 10 | 10/10 | 10/10 | 0/10 | 10/10 官网文章页 | 题录由 Gitee 国内节点从官网 RSS 采集；官网源当前未提供可识别页码。 |
| 经济地理 | 2026 年第 46 卷第 5 期；2026-05-26 | 24 | 24/24 | 24/24 | 0/24 | 24/24 官网文章页 | 已过滤 1 条书评；网页中虽有页码，但当前生成器尚未输出。 |
| 管理世界 | 2026 年第 8 期；2026-07-31 | 12 | 12/12 | 12/12 | 12/12 | 12/12 官网文章页 | 题录由 Gitee 国内节点从官网采集；完整作者与页码取自官网标准引文。 |
